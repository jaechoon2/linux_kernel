...
static int xylonfb_set_par(struct fb_info *fbi)
{
	...
	if (common_data->xylonfb_flags & XYLONFB_VMODE_SET)
		return 0;

	xylonfb_disable_logicvc_output(fbi);
	xylonfb_logicvc_disp_ctrl(fbi, false);

	if (common_data->xylonfb_flags & XYLONFB_PIXCLK_VALID) {
		if (!(common_data->xylonfb_flags & XYLONFB_VMODE_INIT)) {
		...
		rc = xylonfb_hw_pixclk_set(common_data->pixclk_src_id,
	...
	if (common_data->xylonfb_flags & XYLONFB_VMODE_INIT)
		common_data->xylonfb_flags |= XYLONFB_VMODE_SET;
	...
	int memmap)
{
	...
	if (memmap) {
		if (layer_data->xylonfb_cd->xylonfb_flags & XYLONFB_DMA_BUFFER) {
		/* NOT USED FOR NOW! */
	...
	u32 reg_base_phys, int id, int *regfb)
{
	...
	if (!(common_data->xylonfb_flags & XYLONFB_DEFAULT_VMODE_SET)) {
		xylonfb_set_timings(fbi, fbi->var.bits_per_pixel);
		common_data->xylonfb_flags |= XYLONFB_DEFAULT_VMODE_SET;
	}
...
int xylonfb_init_driver(struct xylonfb_init_data *init_data)
{
	...
	common_data->xylonfb_flags |= XYLONFB_VMODE_INIT;
	...
	if (init_data->pixclk_src_id) {
		if (xylonfb_hw_pixclk_supported(init_data->pixclk_src_id)) {
			common_data->pixclk_src_id = init_data->pixclk_src_id;
			common_data->xylonfb_flags |= XYLONFB_PIXCLK_VALID;
		} else {
	...
	common_data->xylonfb_flags |= XYLONFB_MEMORY_LE;
#endif
	...
	common_data->xylonfb_flags &=
		~(XYLONFB_VMODE_INIT | XYLONFB_DEFAULT_VMODE_SET | XYLONFB_VMODE_SET);
	xylonfb_mode_option = NULL;
...
err_fb:
	...
	for (i = layers-1; i >= 0; i--) {
		...
		if (layer_data) {
			if (common_data->xylonfb_flags & XYLONFB_DMA_BUFFER) {
				/* NOT USED FOR NOW! */
